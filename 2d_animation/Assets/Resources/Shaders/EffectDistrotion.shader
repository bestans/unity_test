...
Shader "Custom/Xffect/Distortion" {
	Properties {
		_NoiseTex ("NoiseTex", 2D) = "white" {}           // 絮乱图
		_AreaTex ("AreaTex(Alpha)", 2D) = "white" {} // 区域图：白色为显示区域，透明为不显示区域
		_MoveSpeed  ("MoveSpeed", range (0,1.5)) = 1        // 絮乱图移动速度
		_MoveForce  ("MoveForce", range (0,0.1)) = 0.1      // 絮乱图叠加强度，多张运动纹理叠加后再相乘的系数
	}
